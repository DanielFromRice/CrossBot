...
async def on_ready():
    ...
    await ticBot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="\"xo [command]\""))
...
async def new_game(ctx, member_in: discord.Member):
    ...
    if player1.id == player2.id:
        ...
        if game_data.keys().__contains__(str(player1.id)):
            game_data[str(player1.id)][str(player2.id)] = \
                {"moves": move_list, "turn": True, "sym": "X", "secs": sector_statuses}
        else:
            game_data[str(player1.id)] = \
                {str(player2.id): {"moves": move_list, "turn": True, "sym": "X", "secs": sector_statuses}}
        if game_data.keys().__contains__(str(player2.id)):
            game_data[str(player2.id)][str(player1.id)] = \
                {"moves": move_list, "turn": False, "sym": "O", "secs": sector_statuses}
        else:
            game_data[str(player2.id)] = \
                {str(player1.id): {"moves": move_list, "turn": False, "sym": "O", "secs": sector_statuses}}
        # print(game_data)
...
async def new_game_error(ctx, err):
    if isinstance(err, commands.MissingRequiredArgument):
        ...
        if str(err) == "P1":
            await ctx.send(f"{ctx.message.author}, you already have a game active with this player! Finish or quit your"
                           f" current game to start a new one")
        elif str(err) == "Same":
...
@ticBot.command(name="play", help="Play your turn!\nThe squares are labelled as:\n1 2 3\n4 5 6\n7 8 9\n\nPut the number"
                                  " for the big square (sector) first, then the little square within it."
                                  " Then tag your opponent")
async def play(ctx, sector: int, space: int, opponent: discord.Member):
    ...
    if not game_data.keys().__contains__(str(player.id)) or not game_data.keys().__contains__(str(opponent.id)) or not \
            game_data[str(player.id)].keys().__contains__(str(opponent.id)) or not \
            game_data[str(opponent.id)].keys().__contains__(str(player.id)):
    ...
    if sector_win != "N":
        ...
        sector_list = [[sym, sec] for sec, sym in game_data[str(player.id)][str(opponent.id)]["secs"].items() if
                       sym != "N"]
    ...
    if winner != "N":
        ...
        with io.BytesIO() as image_binary:
            ...
            image_binary.seek(0)
            await ctx.send(
                f"{winner}'s won!!! Congratulations {player}!!! Better luck next time, {opponent}. To play again, use "
                f"the command \"xo new @[opponent]\"",
        ...
        with io.BytesIO() as image_binary:
            ...
            image_binary.seek(0)
            await ctx.send(f"----------------------------------------------")
            await ctx.send(f"Good game {ctx.message.author} and {opponent}! Your game ended with a tie!")
            await ctx.message.delete()
            game_data[str(player.id)].pop(str(opponent.id))
            game_data[str(opponent.id)].pop(str(player.id))
    else:
        ...
        with io.BytesIO() as image_binary:
            ...
            image_binary.seek(0)
            await ctx.send(f"----------------------------------------------")
            await ctx.send(f"{ctx.message.author} just played an {game_data[str(player.id)][str(opponent.id)]['sym']}!",
                           file=discord.File(fp=image_binary, filename='latest_turn.png'))
...
async def play_error(ctx, err):
    if isinstance(err, commands.MemberNotFound):
        ...
        if str(err) == "Wrong Sector":
            await ctx.send(
                f"Might want to rethink that one! You are trying to play in the wrong sector! \nRemember to write your "
                f"move as XO.play <big_grid> <small_grid> @<opponent>")
        else:
...
@ticBot.command(name="info", help="Displays all of your current games")
async def game_info(ctx):
    global game_data
    player = ctx.message.author
    if str(player.id) in game_data.keys():
        if len(game_data[str(player.id)]) > 0:
            await ctx.send(f"Hi {player.display_name}! You have the following active games:")
            for opp_id in game_data[str(player.id)]:
                if game_data[str(player.id)][str(opp_id)]["turn"]:
                    whose_turn = "your"
                else:
                    whose_turn = "their"
                img = UltimateGame.draw_game(game_data[str(player.id)][str(opp_id)]["moves"],
                                             game_data[str(player.id)][str(opp_id)]["secs"])
                with io.BytesIO() as image_binary:
                    img.save(image_binary, 'PNG')
                    image_binary.seek(0)
                    await ctx.send(
                        f"You are currently playing {ticBot.get_user(int(opp_id))} and it is {whose_turn} turn. "
                        f"You are playing as {game_data[str(player.id)][opp_id]['sym']}.")
                    await ctx.send(file=discord.File(fp=image_binary, filename="latest_turn.png"))
        else:
...
@ticBot.command(name="resign", help="Resigns your current game")
async def resign(ctx, other_player: discord.Member):
    player = ctx.message.author
    if game_data[str(player.id)].__contains__(str(other_player.id)):
        ...
        await ctx.send(f"Congrats {other_player}, {player} has resigned from your game!!")
        await write_data()
    else:
