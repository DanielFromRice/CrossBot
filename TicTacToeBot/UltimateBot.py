...
async def write_data():
    ...
    with open("TicTacData/ultimate.txt", 'w') as fil:
...
async def play(ctx, space: int):
    ...
    winner = UltimateGame.check_win(moves)
    if winner == "N":
        img = UltimateGame.draw_game(moves)
        with io.BytesIO() as image_binary:
            img.save(image_binary, 'PNG')
            image_binary.seek(0)
...
async def game_info(ctx):
    ...
    if str(player.id) in game_data.keys():
        if game_data[str(player.id)]["turn"]:
            ...
            whose_turn = "their"
        img = UltimateGame.draw_game(game_data[str(player.id)]["moves"])
        with io.BytesIO() as image_binary:
            img.save(image_binary, 'PNG')
            image_binary.seek(0)
